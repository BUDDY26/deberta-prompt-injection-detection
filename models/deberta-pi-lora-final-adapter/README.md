...
# deberta-pi-lora-final-adapter

LoRA adapter checkpoint for binary prompt injection detection. Adapter weights only — must be
loaded on top of `ProtectAI/deberta-v3-base-prompt-injection` using PEFT.

This is the final adapter output from a two-stage LoRA fine-tuning pipeline (stages 1 and 2).
Stage 3 (Aegis) was not executed for this LoRA run; see the full fine-tuning pipeline results
...
adapter_path = "models/deberta-pi-lora-final-adapter"
