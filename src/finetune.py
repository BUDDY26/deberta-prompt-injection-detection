...
def preprocess_ds2(batch):
    ...
    if "System Prompt" in batch and "User Prompt" in batch:
        combined_text = [
            f"{sys} {usr}"
            for sys, usr in zip(batch["System Prompt"], batch["User Prompt"])
        ]
    ...
    return tokenizer(
        combined_text, truncation=True, padding="max_length", max_length=256
    )
...
train_tok2 = train_ds2.map(
    preprocess_ds2,
    batched=True,
    remove_columns=[col for col in train_ds2.column_names if col != "label"],
)
val_tok2 = val_ds2.map(
    preprocess_ds2,
    batched=True,
    remove_columns=[col for col in val_ds2.column_names if col != "label"],
)
test_tok2 = test_ds2.map(
    preprocess_ds2,
    batched=True,
    remove_columns=[col for col in test_ds2.column_names if col != "label"],
)
