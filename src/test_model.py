    ...
    if "prompt_label" in example:
        ...
        if isinstance(raw_label, str):
            ...
            # Map safe/benign labels to 0
                ...
                keyword in raw_label_lower
                for keyword in ["safe", "benign", "harmless", "legitimate"]
            ):
                label = 0
            else:
...
print(
    f"  Class 0 (Safe):     Precision={precision_per_class[0]:.4f}, Recall={recall_per_class[0]:.4f}, F1={f1_per_class[0]:.4f}, Support={support_per_class[0]}"
)
print(
    f"  Class 1 (Unsafe):   Precision={precision_per_class[1]:.4f}, Recall={recall_per_class[1]:.4f}, F1={f1_per_class[1]:.4f}, Support={support_per_class[1]}\n"
)
...
print(
    classification_report(
        labels, predictions, target_names=["Safe", "Unsafe"], digits=4, zero_division=0
    )
)
...
for idx in random_indices:
    print(f"Example {idx}:")
    print(
        f"  Text: {texts[idx][:200]}..."
        if len(texts[idx]) > 200
        else f"  Text: {texts[idx]}"
    )
