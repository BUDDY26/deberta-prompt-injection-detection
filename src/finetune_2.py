...
ds1 = load_dataset("nvidia/Aegis-AI-Content-Safety-Dataset-2.0")  # continuing training on this dataset

# 2a) Check the dataset structure first
print(f"Dataset splits: {ds1.keys()}")
print(f"Dataset columns: {ds1['train'].column_names if 'train' in ds1 else list(ds1.values())[0].column_names}")
...
def preprocess(batch):
    ...
    tokenized = tokenizer(batch["prompt"], truncation=True, padding="max_length", max_length=256)
    # Convert string labels to integers
    tokenized["labels"] = [1 if label == "unsafe" else 0 for label in batch["prompt_label"]]
    return tokenized
...
train_tok1 = train_ds1.map(preprocess, batched=True, remove_columns=cols_to_remove_train)
...
print(f"Label type: {type(train_tok1[0]['labels'])}, Label value: {train_tok1[0]['labels']}")
...
plot_training_metrics(trainer1, "Aegis-AI-Content-Safety-Dataset-2.0", 3, "training_plots")
